...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const state = {
        mode: 'dbid',              // 'dbid' or 'pid'
        loanAmount: 2500000,       // Default ₹25,00,000 (25 Lakh)
        loanDateStr: '2026-05-19', // YYYY-MM-DD
        ...
        minLimit: 2500000,         // 25 Lakh min for DBID
        maxLimit: 20000000,        // 2 Crore max (default)
        isInclusiveDays: true      // (End - Start + 1)
    };

    // DOM Elements Cache
    const body = document.body;
    const modeDbidSelect = document.getElementById('mode-dbid-select');
    const modePidSelect = document.getElementById('mode-pid-select');
    ...
    function runCalculations() {
        ...
        if (state.loanAmount < state.minLimit) {
            amountValidationMsg.textContent = `Minimum limit is ${formatCurrency(state.minLimit)}`;
            loanAmountInput.parentElement.classList.add('error');
            hasErrors = true;
        } else if (state.loanAmount > state.maxLimit) {
            let limitName = '2 Crore';
            if (state.maxLimit === 50000000) limitName = '5 Crore';
        ...
        // Note: 60 days means (LoanDate + 59 days)
        ...
        if (repaymentDate.getTime() < loanDate.getTime()) {
            repaymentValidationMsg.textContent = `Repayment date cannot be before loan date`;
        ...
        // Inclusive means: RepaymentDate - LoanDate + 1
    ...
    function switchMode(newMode) {
        ...
        if (newMode === 'dbid') {
            body.classList.remove('mode-pid');
            body.classList.add('mode-dbid');
            modeDbidSelect.classList.add('active');
            modePidSelect.classList.remove('active');
            proceedModeBadge.textContent = 'DBID';
            flowStructureLabel.textContent = 'DBID';
            state.minLimit = 2500000; // 25 Lakh min
            state.maxLimit = 20000000; // 2 Crore
            minLimitDisplay.textContent = '₹25,00,000';
            ...
            body.classList.remove('mode-dbid');
            body.classList.add('mode-pid');
            modePidSelect.classList.add('active');
            modeDbidSelect.classList.remove('active');
            proceedModeBadge.textContent = 'PID';
            flowStructureLabel.textContent = 'PID';
            state.minLimit = 100000; // 1 Lakh min
            state.maxLimit = 20000000; // 2 Crore
        ...
        if (state.loanAmount > state.maxLimit) {
            state.loanAmount = state.maxLimit;
            loanAmountInput.value = state.loanAmount.toLocaleString('en-IN');
            loanAmountSlider.value = state.loanAmount;
        } else if (state.loanAmount < state.minLimit) {
            state.loanAmount = state.minLimit;
            loanAmountInput.value = state.loanAmount.toLocaleString('en-IN');
            loanAmountSlider.value = state.loanAmount;
        }

        loanAmountSlider.min = state.minLimit;
        loanAmountSlider.max = state.maxLimit;

        renderFlowSteps();
        runCalculations();
        showToast('Financing Program Changed', `Switched calculation framework to ${newMode.toUpperCase()} successfully.`);
    }

    modeDbidSelect.addEventListener('click', () => switchMode('dbid'));
    modePidSelect.addEventListener('click', () => switchMode('pid'));

    // Loan Amount Inputs Binding (Text Box & Slider Dual-Sync)
    loanAmountInput.addEventListener('focus', () => {
        // Strip commas on focus to make editing easier
    ...
    loanAmountInput.addEventListener('blur', () => {
        ...
        // Clamp bounds if text is entered completely out of bounds (soft correction)
    ...
    loanAmountInput.addEventListener('input', () => {
        ...
        if (raw > 0) {
            state.loanAmount = raw;
            // Instantly sync slider during typing
    ...
    loanDateInput.addEventListener('input', (e) => {
        ...
        // Auto-shift repayment default to maintain consistent timeline if desired
        const oldLoanDate = parseUTCDate(loanDateInput.defaultValue);
        const curLoanDate = parseUTCDate(state.loanDateStr);
    ...
    loanTenureInput.addEventListener('input', (e) => {
        ...
        if (isNaN(val)) {
            state.tenureDays = 60; // Safe fallback in state
            runCalculations();
            return;
        }
    ...
    lockedNavs.forEach(nav => {
        nav.addEventListener('click', (e) => {
            ...
            // Customize modal based on page clicked
            modalPageTitle.textContent = pageLabel;

            let pageDesc = "";
            if (nav.id === 'nav-page-2') {
                pageDesc = "In this step, the Distributor (under DBID) or Retailer (under PID) will submit their active trade invoices for dynamic validation. You can integrate multiple formats (PDF, CSV, image) and check OCR validations.";
            } else if (nav.id === 'nav-page-3') {
                pageDesc = "Our proprietary risk assessment protocol. Evaluate credit ratings, historical payment patterns, cash coverage ratio, and fraud factors to approve invoice discounting in real-time.";
            } else {
    ...
    window.handleProceedAction = function () {
        showToast(
            ...
            `Page 1 Data Saved! Setting up invoice uploads for your ${state.mode.toUpperCase()} application.`
        );

        // Auto show the Page 2 upload invoices modal helper to demonstrate next phase
        setTimeout(() => {
            document.getElementById('nav-page-2').click();
        }, 800);
    };
    ...
    /* ==========================================================================
       Initialization
       ========================================================================== */
    // Sync initial formatted strings to native date input picker displays
